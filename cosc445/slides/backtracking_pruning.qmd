...
def backtrack(partial_solution):
    ...
    for choice in get_choices(partial_solution):
        if is_valid(partial_solution, choice):
            ...
            backtrack(partial_solution)   # explore further
...
```python
def solve_maze(grid, row, col, path, visited):
    ...
    for dr, dc in [(-1,0),(1,0),(0,-1),(0,1)]:  # up/down/left/right
        ...
        if (0 <= nr < R and 0 <= nc < C
            ...
            result = solve_maze(grid, nr, nc,
                                path + [(row, col)], visited)
            if result:
                return result   # propagate success

    visited.remove((row, col))  # ← backtrack: un-visit this cell
    return None
```
...
def subset_sum(nums, target):
    ...
    def backtrack(index, current_sum, chosen):
        ...
        backtrack(index + 1, current_sum + nums[index], chosen)
        chosen.pop()  # ← undo
...
def n_queens(n):
    ...
    def backtrack(row, placement):
        ...
        for col in range(n):
            ...
            backtrack(row + 1, placement)   # try next row
...
def backtrack(index, current_sum, chosen):
    ...
    for i in range(index, len(nums)):
        ...
        backtrack(i + 1, current_sum + nums[i], chosen)
        chosen.pop()
```
...
**Pruning idea:** before placing a digit, compute the set of *still-possible* values for every empty cell. If any cell has **zero** possibilities → prune!
...
With k=3: can we colour A, B, C, D so neighbours differ?
...
def graph_coloring(graph, k):
    ...
    def is_valid(node, c):
        return all(color[nbr] != c for nbr in graph[node])

    def backtrack(node):
        if node == n:
            return True   # all nodes coloured

        for c in range(k):
            if is_valid(node, c):   # ← feasibility pruning built-in
                color[node] = c
                if backtrack(node + 1):
                    return True
                color[node] = -1    # ← backtrack

        return False   # no valid colour found → prune this branch

    return color if backtrack(0) else None
```

Try it: `graph = [[1,2],[0,3],[0,3],[1,2]]`, `k=2` → `[0,1,1,0]` ✓
