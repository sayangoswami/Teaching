...
Simple and correct. But let's trace `fib(5)`:
...
Notice anything? `1, 2, 3, 5, 8, 13, ...` — it's Fibonacci!
...
**This problem teaches the core DP pattern:**  
define `dp[i]` = answer for subproblem of size `i`, build up from base cases.
...
LCS length = **3** (the subsequence `"BCB"`)
...
Items: A(1,1), B(3,4), C(4,5), D(5,7) — capacity W=7
